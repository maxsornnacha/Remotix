...
/** Upper bound on outgoing video bitrate (bps). Caps sender queue buildup on slow links. */
const readRtcMaxVideoBitrate = () => {
  const raw = Number(process.env.NEXT_PUBLIC_RTC_MAX_VIDEO_BITRATE);
  if (Number.isFinite(raw) && raw >= 250_000 && raw <= 40_000_000) return Math.floor(raw);
  return 2_800_000;
};

/**
 * Runtime (post-handshake) latency tuning — does not change signaling / ICE trickle.
 * - Client: shrink receiver playout delay (Chromium `playoutDelayHint`).
 * - Host: keep framerate under congestion; optional `maxBitrate` on first encoding.
 */
export const applyWebRtcLatencyHints = (peer, role) => {
  const pc = peer && peer._pc;
  if (!pc) return;
  if (pc.connectionState === "closed" || pc.signalingState === "closed") return;

  try {
    if (role === "client" && typeof pc.getReceivers === "function") {
    ...
    if (role === "host" && typeof pc.getSenders === "function") {
      const maxB = readRtcMaxVideoBitrate();
      for (const sender of pc.getSenders()) {
        ...
        try {
          ...
          let enc =
            Array.isArray(cur.encodings) && cur.encodings.length > 0
              ? cur.encodings.map((e) => ({ ...e }))
              : [{}];
          if (maxB > 0) {
            enc = enc.map((e, i) => (i === 0 ? { ...e, maxBitrate: maxB } : { ...e }));
          }
          ...
          // negotiation not ready or browser rejected shape
        }
      }
    }
  } catch (_e) {
...
/** Re-apply hints as tracks/receivers attach (timing varies after media flows). */
export const runWebRtcLatencyWarmup = (peer, role) => {
  const delays = [0, 80, 250, 700, 2000];
  const ids = delays.map((ms) => window.setTimeout(() => applyWebRtcLatencyHints(peer, role), ms));
  return () => {
...
const pickSelectedCandidatePair = (statsEntries) => {
  const candidatePairs = statsEntries.filter(
    (entry) => entry.type === "candidate-pair",
  );
  const selectedPair =
    candidatePairs.find((pair) => pair.selected) ||
    candidatePairs.find(
      (pair) => pair.nominated && pair.state === "succeeded",
    ) ||
    null;
  ...
  const local = statsEntries.find(
    (entry) =>
      ...
      entry.id === selectedPair.localCandidateId,
  );
  const remote = statsEntries.find(
    (entry) =>
      entry.type === "remote-candidate" &&
      entry.id === selectedPair.remoteCandidateId,
  );
